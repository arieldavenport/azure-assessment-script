...
function Invoke-OptionR_ReadinessReport {
    ...
    if ($sampleVm) {
        $m = Get-AzMetric -ResourceId $sampleVm.Id -MetricName 'Percentage CPU' -TimeGrain 01:00:00 -StartTime (Get-Date).AddDays(-7) -EndTime (Get-Date) -AggregationType Average -ErrorAction SilentlyContinue
